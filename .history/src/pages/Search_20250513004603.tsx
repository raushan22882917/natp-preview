  ...
  const performSearch = useCallback(async (query: string) => {
    ...
    try {
      ...
      const filteredResults = allTrademarks.filter(trademark => {
        ...
        // 2. Exact word match for mark words (case sensitive) - PRIORITIZED
        if (!matchFound && trademark.mark) {
          ...
          // Also check if the query is contained within the mark (for partial matches)
          if (!matchFound && trademark.mark.includes(query)) {
            matchFound = true;
            matchReason = `Match found by partial mark match: ${query} in ${trademark.mark}`;
          }
        }
  ...
  const features = [
    {
      ...
      description:
        "Use the Quick Search tool to locate published trademarks by mark name, application number, or owner name - prioritizing search by mark for more relevant results.",
    },
    {
  ...
  const faqs = [
    {
      ...
      answer:
        "You can find trademarks by searching with the mark name, application number, or owner name. Enter an exact word from the trademark mark (e.g., 'NIKE'), the exact application number (e.g., 97654321), or an exact word from the owner name (except 'LLC') in the search field and select 'Submit'. The search prioritizes matches by mark first, then application number, and finally owner name. The search is case sensitive for application numbers and mark words, but case insensitive for owner names. For example, searching for 'raushan' will match both 'raushan' and 'RAUSHAN' in owner names. For application numbers, all digits must match exactly. Each result links to a detailed article for further information.",
    },
    {
        ...
        <div className="max-w-3xl mx-auto">
          ...
          <form
            ...
            <div className="relative w-full">
              <Input
                type="text"
                placeholder="Search using the mark, application number, or owner's name."

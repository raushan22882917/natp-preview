...
export const updateTrademarkKeywords = async (
  ...
  try {
    ...
    // Convert keywords to a string for storage
    // This is a workaround for databases that don't support arrays well
    const keywordsString = validKeywords.join(',');

    // Update the trademark with the keywords string
    const { error } = await supabase
      .from('trademarks')
      .update({
        // Store as a comma-separated string in the description field temporarily
        // This is just a workaround until the database schema is updated

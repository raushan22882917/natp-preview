...
export const updateTrademarkKeywords = async (
  ...
  try {
    ...
    // First try the direct approach with array
    try {
      ...
      // If direct approach fails, try with JSON string
      console.log("Direct array update failed, trying with JSON string");
    } catch (err) {
      console.log("Error with direct array update:", err);
      // Continue to fallback approach
    }

    // Fallback: Use a JSON string approach
    const keywordsJson = JSON.stringify(validKeywords);

    // First update with a JSON string in a temporary field
    ...
    if (jsonError) {
      console.error('Error updating keywords_json:', jsonError);
      return { success: false, error: jsonError.message };
    }

    // Then use a database function or RPC to convert the JSON to an array
    // This is a placeholder - you would need to create this function in your database
    const { error: rpcError } = await supabase.rpc('convert_keywords_json_to_array', {
      trademark_id: trademarkId
    });

    if (rpcError) {
      console.error('Error converting keywords:', rpcError);

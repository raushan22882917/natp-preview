import { Navbar } from "@/components/Navbar";
import { Button } from "@/components/ui/button";
import { Link } from "react-router-dom";
import { Footer } from "@/components/Footer";
export default function SEO() {
  return (
    <div className="min-h-screen font-inter">
      <Navbar />

      <div className=" min-h-[200px] flex items-center justify-center">
        <div className=" max-w-5xl   flex gap-5">
          <h1 className="text-5xl font-semibold text-[#333747]  mb-4">
            Search Engine Optimization
          </h1>
          <p className="w-1/2 text-[#333747] text-lg font-semibold">
            Our SEO services enhance your brand's visibility on search engines,
            making it easier for potential customers to discover you. By
            optimizing the content and targeting relevant keywords, we help you
            attract organic traffic that drives growth and builds trust.
          </p>
        </div>
      </div>

      <section className=" py-16  my-10">
        <div className="max-w-5xl mx-auto ">
          <p className="text-[#333747] text-sm mb-6">visibility</p>
          <h2 className="text-4xl font-semibold  text-[#333747] mb-6">
            Unlock Your Brand's Potential with SEO
          </h2>
          <p className="text-[#333747] font-semibold text-lg max-w-5xl  mb-16">
            Search Engine Optimization (SEO) is essential for new brands aiming
            to enhance their online presence. Our services focus on optimizing
            the content, targeting the right keywords, and improving site
            structure to ensure you rank higher in search results. By leveraging
            SEO, you can attract potential customers organically and build
            lasting trust.
          </p>

          <div className="grid md:grid-cols-3 gap-8 mb-[12rem]">
            {/* Card 1 */}
            <div className="flex flex-col ">
              <img
                src="https://wtpregister.com/images/tpm/box.png"
                className="
            w-10 h-10"
                alt=""
              />
              <h3 className="text-[26px] font-semibold mt-4 text-[#333747] mb-6">
                Comprehensive Content Optimization for Your Brand
              </h3>
              <p className="text-[#333747] text-[15px] font-semibold ">
                We refine your website's content to resonate with your target
                audience.
              </p>
            </div>

            {/* Card 2 */}
            <div className="flex flex-col">
              <img
                src="https://wtpregister.com/images/tpm/box.png"
                className="
            w-10 h-10"
                alt=""
              />
              <h3 className="text-[26px] font-semibold mt-4 text-[#333747] mb-6">
                Strategic Keyword Targeting for Maximum Reach
              </h3>
              <p className="text-[#333747]  text-[15px] font-semibold">
                Our team identifies and implements the most effective keywords.
              </p>
            </div>

            {/* Card 3 */}
            <div className="flex flex-col">
              <img
                src="https://wtpregister.com/images/tpm/box.png"
                className="
            w-10 h-10"
                alt=""
              />
              <h3 className="text-[26px] font-semibold mt-4 text-[#333747] mb-6">
                Enhanced Site Structure for Better Navigation
              </h3>
              <p className="text-[#333747]  text-[15px] font-semibold ">
                We optimize our site’s architecture to improve user experience.
              </p>
            </div>
          </div>

          {/* Bottom Section */}
          <div className="flex flex-col md:flex-row justify-between max-w-5xl mx-auto">
            <div className="mb-6 md:mb-0">
              <h3 className="text-[40px] font-semibold text-[#333747] mb-2">
                Boost Your Brand's Online Presence
              </h3>
            </div>
            <div className="flex flex-col w-1/2">
              <div className="text-[#333747] mb-6 text-lg font-semibold">
                Ready to elevate your brand's visibility? Contact us today for a
                personalized consultation and discover how our SEO strategies
                can drive organic traffic to your website.
              </div>
              <button className="mr-auto bg-[#00A3E0] mt-5 text-white shadow-md py-3 px-8">
                Contact
              </button>
            </div>
            <div></div>
          </div>
        </div>
      </section>

      <Footer />
    </div>
  );
}
